...
class ThemeManager {
    ...
    getTheme() {
        return {
            ...
            backgroundImageSwitch: localStorage.getItem('backgroundImageSwitch') || 'false'
        };
    }
    ...
    backgroundImageChoose() {
        ...
        input.type = 'file';
        input.accept = 'image/*';
        input.onchange = async (e) => { // 保持 onchange 为 async 函数
            ...
            reader.onload = (event) => {
                ...
                img.onload = () => {
                    if (typeof mdui !== 'undefined' && typeof mdui.getColorFromImage === 'function') {
                        mdui.getColorFromImage(img).then(color => {
                            console.log(color);
    ...
    backgroundImageSwitchChange() {
        ...
        if (backgroundImageSwitch.checked) {
            try {
                ...
                backgroundImageSwitch.checked = false; // 如果加载失败，取消选中状态
            }
        } else {
